...
def orthogonalizegroup (poly_dict, group, index, angle, points_coords, points_id, threshold):
	...
	get_index_group = {}
	for gr in xrange(len(index)):
		for point in xrange(len(index[gr])):
			no = str(index[gr][point])
			get_index_group[no] = gr

	for OID in group[1]:
		fixed_points = []
		for partNO in xrange(len(points_id[OID])):
			for pntNO in xrange(len(points_id[OID][partNO])):
				if pntNO not in fixed_points:
					group_no = get_index_group[str([OID, partNO, pntNO])]
					loop = len(points_id[OID][partNO])

					p1_no = pntNO
					p_prev_no = (pntNO - 1) % loop
					p2_no = (pntNO + 1) % loop
					p_next_no = (pntNO + 2) % loop
					...
					while not ok_1 or not ok_2:
						...
						p1_id = points_id[OID][partNO][p1_no]
						p2_id = points_id[OID][partNO][p2_no]
						p_prev_id = points_id[OID][partNO][p_prev_no]
						p_next_id = points_id[OID][partNO][p_next_no]
					...
					if trigger == 1:
						...
						while p_corr_no != p2_no:
							p_corr_id = points_id[OID][partNO][p_corr_no]
						...
						# fixed_points += [p1_id, p2_id]

	# Перезапись координат в классе.
	for oid in points_id:
		for prt in xrange(len(points_id[oid])):
			for pnt in xrange(len(points_id[oid][prt])):
				poly_dict[oid]['geom'][prt][pnt][0] = points_coords[points_id[oid][prt][pnt]][0]
				poly_dict[oid]['geom'][prt][pnt][1] = points_coords[points_id[oid][prt][pnt]][1]
